...
PROGRAM MAIN
  ...
  ! 局部变量
  REAL :: TSTRT              ! 记录程序开始运行的 CPU 时间
  REAL :: TEND               ! 记录程序运行结束的 CPU 时间
  ...
  CALL CPU_TIME(TEND)
  WRITE(U_LOG, "('----------------------------------------')")
  WRITE(U_LOG, "('模拟结束，用时 ', ES12.3, ' 秒')") TEND - TSTRT
  WRITE(U_LOG, "()")
  IF (IRUNMOD == 1) THEN
    WRITE(*, "('----------------------------------------')")
    WRITE(*, "('模拟结束，用时 ', ES12.3, ' 秒')") TEND - TSTRT
    WRITE(*, *)
  END IF
